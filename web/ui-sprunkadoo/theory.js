...
/** Set of pitch classes (0..11) in `key`'s scale. Used by the
 *  Performer Sprunki's auto-snap path — every incoming MIDI note
 *  gets its pitch class checked against this set; if it's not
 *  in the scale, the snap shifts it to the nearest scale tone.
 *  Returns an empty Set if the key/mode isn't recognized so the
 *  snap path falls through to "leave the note alone." */
export function pitchClassesForKey(key) {
  ...
  return new Set(intervals.map((iv) => (keyRoot + iv) % 12));
}
