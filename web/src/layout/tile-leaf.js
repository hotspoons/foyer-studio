...
const VIEW_CATALOG = new Map(VIEWS.map(v => [v.id, v]));
VIEW_CATALOG.set("preview", { id: "preview", label: "Preview", icon: "document" });
...
export class TileLeaf extends LitElement {
  ...
  render() {
    const meta = VIEW_CATALOG.get(this.leaf.view) || { id: this.leaf.view, label: this.leaf.view, icon: "document" };
    return html`
      <header
        @click=${() => this._focus()}
  ...
  _renderMenu() {
    const items = Array.from(VIEW_CATALOG.values()).filter(v => v.id !== "preview");
  ...
  _renderBody() {
    ...
    switch (v) {
      case "mixer":    return html`<foyer-mixer .session=${session}></foyer-mixer>`;
