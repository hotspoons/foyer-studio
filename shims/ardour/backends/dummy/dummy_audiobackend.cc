/*
 * Foyer Studio — vendored fork of Ardour's "None (Dummy)" audio
 * backend. The fork lives here so that Foyer can run against an
 * unmodified Ardour 9.x build (Debian package, AppImage, official
 * .deb, locally compiled tree) instead of needing the upstream
 * source patched. Ardour's AudioBackend ABI is the only API surface
 * this depends on; everything we touched stays inside the .so.
 *
 * Substantive changes from upstream:
 *
 *   1. Backend name: "None (Dummy)" → "Foyer Dummy" so both
 *      backends can coexist in Ardour's AMS picker without
 *      collision and a session's saved engine state is
 *      unambiguous.
 *   2. main_process_thread() uses an absolute-time deadline +
 *      `clock_nanosleep(CLOCK_MONOTONIC, TIMER_ABSTIME)` on Linux
 *      to keep the audio clock locked to wall clock on
 *      SCHED_OTHER threads (containers without CAP_SYS_NICE —
 *      Cloud Run gen2, plain `docker run`). The original
 *      `Glib::usleep(...)` accumulates scheduling drift on non-RT
 *      threads — search "_target_cycle_us" below for the why.
 *      Other platforms keep the original drift-accumulating path.
 *
 * Sync-with-upstream policy: this file is a hand-rebased fork. Pull
 * `libs/backends/dummy/dummy_audiobackend.{cc,h}` and
 * `dummy_midi_seq.h` from a fresh Ardour 9.x release once a year or
 * when chasing a specific bug; reapply the two changes above.
 *
 * Original copyright (preserved per GPL-2-or-later requirements):
 *
...
// Foyer Studio: rebranded so this backend appears alongside the
// upstream "None (Dummy)" in Ardour's AMS picker without colliding,
// and so users know which one their session was saved against. The
// only other functional change vs. upstream is the absolute-time
// sleep in main_process_thread() (search "_target_cycle_us") that
// keeps the audio clock locked to wall clock on non-RT scheduling
// — see the comment at that site for the why.
