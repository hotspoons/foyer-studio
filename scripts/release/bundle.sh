...
FOYER_DESKTOP_BIN="${FOYER_DESKTOP_BIN:-$ROOT_DIR/target/release/foyer-desktop}"
SHIM_LIB="${SHIM_LIB:-$ROOT_DIR/shims/ardour/cmake-build/libfoyer_shim.$shim_ext}"
SKIP_SHIM="${FOYER_SKIP_SHIM:-0}"
# foyer-desktop is Linux-only in this release (macOS shell needs a
# separate code-signing / Cocoa polish pass). Skip on macOS and skip
# automatically when the binary isn't where we expect it (e.g. the
# user ran `just release-bundle` without first building it).
...
# Linux-only: foyer-desktop binary + icon + .desktop template.
if [ "$SKIP_DESKTOP" != "1" ]; then
    cp "$FOYER_DESKTOP_BIN" "$staging/foyer-desktop"
    chmod 0755 "$staging/foyer-desktop"

    # Icon — the mixer-fader logo (`web/logo.svg`), NOT the smaller
    # web favicon. The logo reads as "DAW" at the sizes a Linux app
    # grid / dock / activities panel actually shows. SVG works in
    # every modern menu (GNOME / KDE / XFCE / sway-launchers all
    # consume `~/.local/share/icons/hicolor/scalable/apps/`).
    ...
    # alone) means an admin doing a manual / packaged install gets
    # the same XDG entry layout for free.
